...
import com.syneygy.fillnow.Jpfmsdutymanager.fillnowdutymanager1.hardware.interfaces.OnAtgQueueCompleted;
import com.syneygy.fillnow.Jpfmsdutymanager.fillnowdutymanager1.hardware.synergy.ServerATG285;

import java.util.concurrent.ExecutionException;

public class CommandReadATGQueue {

    private static OnAtgQueueCompleted onTxnCompleted;
    ...
    private static CommandReadATGQueue readTxnQueue;

    public CommandReadATGQueue(String[] commandsListReadTxn, OnAtgQueueCompleted onTxnCompleted) {
    ...
    public static CommandReadATGQueue getInstance(String[] commandsList, OnAtgQueueCompleted onTxnQueueCompleted) {
        if (readTxnQueue == null) {
            readTxnQueue = new CommandReadATGQueue(commandsList, onTxnQueueCompleted);
            return readTxnQueue;
        } else {
                ...
                try {
                    if (executeCommandOneByOne(commandsListReadTxn[currentCommandTxn])) {
                        ...
                        if (commandsListReadTxn.length == 1) {
                            onTxnCompleted.onAtgQueueEmpty(true, receiveTxn);
                            return;
                        }
                        currentCommandTxn++;
                        isReceivedTxn = false;
...
//                    }
                    ...
                    if (commandsListReadTxn != null && byteData!=null) {
                        isReceivedTxn = true;
                        onTxnCompleted.OnAtgQueueCommandCompleted(currentCommandTxn, new String(byteData));
                    } else {
                        isReceivedTxn = false;
                    }
                }
