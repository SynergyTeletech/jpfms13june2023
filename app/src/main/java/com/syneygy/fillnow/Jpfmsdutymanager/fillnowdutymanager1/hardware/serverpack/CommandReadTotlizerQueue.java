...
public class CommandReadTotlizerQueue {
        private static OnTxnQueueCompleted onTxnCompletedvolTot;
        private static String[] commandsListReadvolTot;
        private static String receiveTxn;
        private static boolean isReceivedTxn;
        private static int currentCommandTxn;
        private static CommandReadTotlizerQueue readTxnQueue;

        public CommandReadTotlizerQueue(String[] commandsListReadTxn, OnTxnQueueCompleted onTxnCompleted) {
            TerminateCommandChain();
            this.commandsListReadvolTot = commandsListReadTxn;
            this.onTxnCompletedvolTot = onTxnCompleted;
            currentCommandTxn = 0;

        }

        public static CommandReadTotlizerQueue getInstance(String[] commandsList, OnTxnQueueCompleted onTxnQueueCompleted) {
            if (readTxnQueue == null) {
                readTxnQueue = new CommandReadTotlizerQueue(commandsList, onTxnQueueCompleted);
                return readTxnQueue;
            } else {
                readTxnQueue.commandsListReadvolTot = commandsList;
                readTxnQueue.onTxnCompletedvolTot = onTxnQueueCompleted;
                return readTxnQueue;
            }
        ...
        public void doCommandChaining() {
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    try {
                        if (executeCommandOneByOne(commandsListReadvolTot[currentCommandTxn])) {
                            Log.e("kamalCamand",commandsListReadvolTot[currentCommandTxn]);
                            Log.d("CommandInRead", "Cleared Phase One by " + Commands.getEnumByString(String.valueOf(commandsListReadvolTot[currentCommandTxn])) + "" + ",Incrementing Value");
                            if (commandsListReadvolTot.length == 1) {
                                onTxnCompletedvolTot.onTxnQueueEmpty(true, receiveTxn);
                                return;
                            }

                            currentCommandTxn++;
                            isReceivedTxn = false;
                            if (currentCommandTxn > commandsListReadvolTot.length - 1) {
                                onTxnCompletedvolTot.onTxnQueueEmpty(true, receiveTxn);
                                Log.d("queExceedChainAfter", String.valueOf(Commands.getEnumByString(String.valueOf(commandsListReadvolTot[currentCommandTxn - 1])) + "length= " + commandsListReadvolTot.length));
                                return;
                            } else {
...
//                        isReceivedTxn = true;
//                    }
                        if (commandsListReadvolTot != null && encodeHexString(byteData).contains("7f")) {
                            isReceivedTxn = true;
                            onTxnCompletedvolTot.OnTxnQueueCommandCompleted(currentCommandTxn, new String(byteData));
                        } else {
                            isReceivedTxn = false;
                        }
                    }
